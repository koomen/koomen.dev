...
But in the Gmail case, this AI assistant is supposed to represent _me_. These are _my emails_. I want them written in _my_ voice, not the one-size-fits-all slop voice designed by a committee of Google product managers and lawyers. 
...
## Render unto the developer...

My core contention is this: when an LLM agent is acting on my behalf I, as the world's foremost expert on me, should be allowed to teach it how to do that.

The System and User prompt components are useful abstractions, but not for dividing work between the developer and the user. The System Prompt is not code, is the interface for teaching an AI "agent" (or "assistant") who it is and how it operates in the general case. The User Prompt is the interface for describing a specific task. 

In the email-writing case, I want to use the System Prompt to teach the LLM how I write emails. If I want help writing an email draft from scratch I'd use the User Prompt to do that, but if I just want help responding to an incoming email, I shouldn't even need to write the User Prompt--it's just the email I'd like a response written for.
