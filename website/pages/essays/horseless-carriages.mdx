...
The System and User prompt components are useful abstractions, but not for dividing work between the developer and the user. The System Prompt is not code, is the user interface for teaching an AI "agent" (or "assistant") who it is and how it operates in the general case. The User Prompt is the .  

In the email-writing case, the System Prompt teaches the agent 



My core contention in this essay is that AI applications that do work my behalf should allow me to see and edit the entire prompt, including the System Prompt. 
