...
Does this mean I always want to write my own System Prompt from scratch? No. I've been using Gmail for twenty years; Gemini should be able to write a draft prompt for me using my emails as reference examples. I'd like to be able to see that prompt and edit it, though, because the way I write emails and the people correspond with change over time.
...
This is the case for every accounting team in every company I've ever worked for. It's why so much of finance still run on excel: it's a general tool that can handle an infinite number of specific use cases.

I conclude from this that in most AI apps the System Prompts will be written and maintained by users, not software developers or even domain experts hired by developers.

Most AI apps will be _agent builders_, not agents.
...
The reason, of course, is that I prefer my emails to be as short as possible, which means any email written in my voice will be roughly the same length as the User Prompt that describes it. This has been my experience LLMs and writing in general. Surprisingly, generative AI is not actually that useful for generating text.

LLMs are really good at reading text and transforming it into other text, and that's what I'd like to use an agent for. Let's revisit our email-reading agent demo:
...
Thanks to my colleagues at YC for reading drafts of this essay.
