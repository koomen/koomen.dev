...
When I use AI to build software I feel like I can accomplish almost anything I can imagine very quickly. AI feels like a power tool. It's a lot of fun.
...
As you can see, Gemini has produced a plausible sounding draft that fits the prompt I submitted perfectly. The problem is that this doesn’t sound anything like an email that I would actually write. If I'd written this email myself, it would have sounded something like this:
...
This could not be further from the truth: Gemini is an astonishingly powerful model and more than capable of writing good emails. The problem is that the Gmail team designed an app that prevents it from doing so.

## A better email assistant

Before we explore the problems with Gmail's AI assistant, let's take a look at a simple demo of an email assistant that is more useful:


<EmailLabeler showPromptAndTools={false} />
...
Of course, this is a contrived example chosen to make a point.




I'll pause here to address an obvious objection: the email I would have written is actually shorter than my original prompt. This is a contrived example chosen to make a point about the tone of Gmail's assistant. I'll address the fact that LLMs aren't actually that helpful for writing emails from scratch [later on in this essay](#an-agent-for-reading-my-email).
...
<EmailDraftWriter 
    ...
    caption="A dummy version of Gmail's draft generator that uses gpt-4o-mini under the hood"
/>

But this is obviously dumb. Writing these instructions took much longer than it would have taken me to write the email myself in the first place. Remarkably, the Gmail team has shipped a product that perfectly captures the experience of managing an underperforming employee.
...
If, instead of forcing me to use their one-size-fits-all System Prompt, Gmail let me to write my own, it would look something like this:
...
<EmailDraftWriter 
    ...
    caption="Depending on the System Prompt, gpt-4o-mini returns dramatically different responses to the same User Prompt"
/>
...
And of course in most cases the only way for developers to do this profitably is to build one piece of software for a large group of users. Developers are used to building one-size-fits-all software, and we're used to using it.
