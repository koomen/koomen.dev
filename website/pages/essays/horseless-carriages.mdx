...
I am beginning to suspect that many of these apps are the "horseless carriages" of the AI era. They're bad because they mimic old ways of building software that unnecessarily constrain the power of the incredible AI models under the hood. As a result they've made it easy for a lot of people to dismiss AI.
...
Here's a dummy version of Gmail's email draft writer that uses OpenAI's gpt-4o-mini model to write emails:
...
Most AI apps should not be agents, they should be _agent builders_.

## ...and unto the developer what is the developer's

If developers aren't writing prompts, what will they do? 

Well for one, they'll creates UIs for building agents that operate in a particular domain, like an email inbox or a general ledger. 
...
Users also need an interface for reviewing an agent's work and iterating on their prompts, similar to little dummy email agent builder I included above. This interafce gives the user the fast feedback loop they need to train an agent to perform a task reliably. 
...
Tools provide the security layer for agents. Whether or not an agent can do a particular thing is determined by which tools it has access to. It is much easier to enforce boundaries with than it is to enforce boundaries between the System and User Prompts. I suspect that in the future we'll look back and laugh at the idea that a "prompt injection" was something to be concerned about.

## An agent for _reading_ my email
...
The Gmail team built a horseless carriage because set out to add AI to the email client they already had, rather than ask what an email client would look like if it were designed from ground up with AI. Their app is a little bit of AI jammed into an interface designed for mundane human labor rather than an interace designed for automating mundane labor.
