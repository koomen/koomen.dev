...
My core contention in this essay is this: when an LLM agent is acting on my behalf I, as the world's foremost expert on me, should be allowed to teach it how to do that.

The System and User prompt components are useful abstractions, but not for dividing work between the developer and the user. The System Prompt is not code, it is the interface for teaching an AI "agent" (or "assistant") who it is and how it operates in the general case. The User Prompt is the interface for describing a specific task to be accomplished. 
