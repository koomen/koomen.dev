...
Even in a world where I could write my own system prompt, it's unlikely that an email-drgsafting agent would save me much time on the email to my boss. The reason, of course, is that I prefer my emails to be as short as possible, which means any email written in my voice will be roughly the same length as the User Prompt that describes it. 
...
It's not hard to imagine how much time an email-reading agent like this could save. This prompt took me five minutes to write and it already seems to do a better job of detecting spam than Gmail's built-in spam filter. It could use my email-writing agent to write draft responses to every important message. It could auto-archive everything I don't need to read and summarize everything I do.
