...
The System and User prompt components are useful abstractions, but not for dividing work between the developer and the user. The System Prompt is not code, it is the interface for teaching an AI "agent" who it is and how it operates in the general case. It is the _context_ an agent needs in order to do its job. The User Prompt is the interface for describing a specific task to be accomplished by the agent.
...
More generally: **the context an agent needs in order to do X should be written by an expert in X**.
