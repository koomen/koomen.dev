...
This is the "old world thinking" that I think led the Gmail team to design their AI assistant this way: _it's the developer's job to build the software, and the System Prompt is part of the software, so it must be the developer's job to write the System Prompt too._ Looking back at the way we've always build software, this is a perfectly reasonable assumption.

But these are _my emails_, and this AI assistant is supposed to represent _me_. I want them written in _my_ voice, not the one-size-fits-all voice designed by a committee of Google product managers and lawyers. In the old world I'd have to accept the one-size-fits-all version because the only alternative was to write my own code, and writing code is hard.

In the new world, however, I don't need a middleman tell a computer what to do anymore. I just need to be able to write my own System Prompt, and writing System Prompts is easy!

## Good AI apps

My core contention in this essay is that AI applications that do work on my behalf should allow me to edit the whole prompt. 
